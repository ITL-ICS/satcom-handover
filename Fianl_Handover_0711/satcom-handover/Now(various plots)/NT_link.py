...
class NT_Link:
    def __init__(self, sat_geo_position, obs_geo_position): # sat_geo_position: 좌표 하나, obs_geo_position: 튜플 타입
        self.sat_geo_position = sat_geo_position
        self.obs_geo_position = obs_geo_position
    
    def calculate_distance(self): # 두 점간 거리 계산
        
        for element in self.obs_geo_position: # obs_ecef가 tuple 형식이어서 for문으로 원소 하나씩 접근시킴
            obs_ecef = element.geo2ecef () 
